...
function isDictSpam(item) {
  ...
  if (/^.\s*[（(]汉字[）)]/.test(item.title||"")) return true;
...
function searchBing(eng, q, limit) {
  ...
  function _filter(items) {
    var clean = [];
    for (var i = 0; i < items.length; i++) if (!isDictSpam(items[i])) clean.push(items[i]);
    return clean;
  }
  ...
  try {
    ...
    if (items.length === 0) {
      ...
      if (chars.length >= 2) {
        ...
        for (var li = 0; li < lastTry.length; li++) items.push(lastTry[li]);
      }
    }

    // 4. 标记SPA结果（不删除，但让AI知道这些爬不了）
...
// ========== 路由 ==========
var ROUTES = {
  "@game":  { o: ["bwiki_ys","bwiki_sr","bwiki_ak","bwiki_zzz","bwiki_bh3","bwiki_blhx","bwiki_zmd","bwiki_ww","bwiki_gbf","prts_wiki","moegirl","bilibili","bing"], min: 3 },
