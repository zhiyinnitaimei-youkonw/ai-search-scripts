# 作者的话
嗨，大家好，我是梅影寒窗。做这个项目的主要原因是答应了群里一个人的请求，所以做了**RikkaHub**的配套搜索引擎，也就是custom JS部分。注意，由于这个使用的是quickJS，在windows上虽然可以用node运行，但是是异步的，有些逻辑需要修改，你可以让AI改一下。不过你都用电脑跑AI了，联网直接用playwright plugin就好了。本项目爬取到的所有信息都不会上传，放心吧，我不是大数据专业的不会用你的信息（也许以后做市场调研会？）。
关于搜索方面，其实本项目跟python专业课的课设项目有点像，因为国内免费的搜索引擎只有bing RSS，而且这玩意也要说再见了，而唯一免费API也只有B站，所以可以理解为"B站弹幕评论热点分析"的延伸版。除了这俩外，还有掘金和一个星铁数据库和原神数据库（感谢每一个无偿做数据库的人），其余数据库欢迎大家补充。
第一版就暂时到这里，后续如果有什么我喜欢玩的新游戏，我会再更新。

### 温馨提醒：注意自身使用的AI智力，智力越高越遵守规则。理论一次复杂搜索，如“原神最近几个版本有什么新角色，强度怎么样？”仅消20K token即可生成回答。如果效果不好，往往是你的模型智力不够，推荐至少DeepSeek-v4-pro使用。

## 超级大更新1.0
超超超超超级大幅度减少了搜索的token消耗，各种无效的返回很令人厌烦？没事，我做了一个搜索结果浓缩，灵感源自于卖课的毒鸡汤，做了一个词向量来实现精炼搜索返回。比如你之前搜索昔涟，往往会返回一些无效评论，比如“我老婆真好看”（确实是我老婆），“感觉不如————”诸如此类的冗余信息。现在会清洗为 ≥2次出现的关键词+来源地址 以方便去爬取验证。
...
**本站提供的资源均来自网络，版权争议与本站无关。© 2026 梅影寒窗。允许个人使用、修改、分发。严禁出售及商业用途。**
...
| 文件 | 用途 |
|------|------|
| `search-v8.0.js` | 16引擎中文聚合搜索，含游戏名感知路由剪枝 |
...
将 `search-v7.6.js` 和 `crawl-v2.5.js` 部署到RikkaHub的个人配置搜索服务，`prompt.md` 和 `SKILL.md` 粘贴到 AI 的系统提示词以及新增skill。

考虑到某些人不会用，这里给出逐步说明：

*前提：你的RikkaHub已经配置好了AI*

1. 点击主界面下方左2的图标*一般为一个放大镜*

2. 点击**网络搜索**右边的六边形螺丝*设置*

3. 点击右上角‘+’号

4. 在添加供应商列表往下滑，找到最底下那个**Custom JS**，点击并按**确认**

5. 此时自动回到搜索服务页，点击CustomJS右侧的**竖三点**，点击**编辑**，进入配置页

6. 名称自定义，**搜索脚本**输入框先清除里面的代码，复制`search-v7.6.js`内的内容到输入框中去*手机会由于单次复制上限导致无法成功，此时建议分段复制粘贴或电脑连接手机来实现一次性粘贴*;然后，**抓取脚本**也复制`crawl-v2.5.js`的内容到输入框中去。*注：抓取脚本输入框内初始为空，灰色的代码是建议，不是实际代码，删不掉的*

7. 返回主界面，然后屏幕右滑或点击左上角，弹出侧边栏。点击设置，进入设置页。点击**扩展管理**，进入扩展管理页。点击**Agent Skills**，进入代理技能页。点击右下角‘+’号，选择从文件导入→选择SKILL.md or 选择从github导入，输入我的github网址：‘  https://github.com/zhiyinnitaimei-youkonw/ai-search-scripts’。出现**web-search-tookit**后返回两次到设置页

8. 点击助手，点击默认助手（可自己改名）进入助手设置页。点击提示词，进入提示词设置页，在**系统提示词**下的输入框里输入`prompt.md`里的内容 or 点击右上角的*一张纸以及一个←*，选择从下载过来的`prompt.md`。

9. 最后返回一级到助手设置页，点击扩展管理，左滑到Skills部分，启用**web-search-tookit***点击右侧按钮*

10. 回到主界面，下方左二图标，在弹出的搜索设置里点击**Custom JS**，然后返回，最终放大镜图标变为**C**，一切成功，开始对话吧。

*补充说明：由于脚本大小过大，有时手机性能不好的情况下脚本配置会闪退。此时建议用电脑控制手机，也就是通过‘手机连接’来一次性输入代码，然后尽量避免再回到脚本编辑页造成闪退。如果已经发生闪退也不用慌，下次打开时时打开、退出、再打开就好了*
