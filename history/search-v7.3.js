...
function translateForBing(q) {
  ...
  // 替换游戏名为英文，保留其余中文关键词（Bing能处理中英混合）
  var en = GAME_EN[bestKey];
  var rest = q.replace(bestKey, "").replace(/^\s+|\s+$/g, "");
  // 尝试：全英文 > 英文+中文
  var candidates = [en + (rest ? " " + rest : "")];
  // 如果剩余部分是纯中文，也尝试纯英文
  var restEn = rest.replace(/[一-鿿]+/g, "").trim();
  if (restEn !== rest) candidates.push(en + " " + en); // 兜底：游戏名重复
  return candidates;
}
