# 作者的话
哈喽大家好，我是梅影寒窗。做这个项目的主要原因是答应了群里一个人的请求，所以做了**RikkaHub**的配套搜索引擎，也就是custom JS部分。注意，由于这个使用的是quickJS，在windows上虽然可以用node运行，但是是异步的，有些逻辑需要修改，你可以让AI改一下。不过你都用电脑跑AI了，联网直接用playwright plugin就好了。本项目爬取到的所有信息都不会上传，放心吧，我不是大数据专业的不会用你的信息（也许以后做市场调研会？）。
关于搜索方面，其实本项目跟python专业课的课设项目有点像，因为国内免费的搜索引擎只有bing RSS，而且这玩意也要说再见了，而唯一免费API也只有B站，所以可以理解为"B站弹幕评论热点分析"的延伸版。除了这俩外，还有掘金和一个星铁数据库和原神数据库（感谢每一个无偿做数据库的人），其余数据库如果你愿意与我分享，可以私信我。如果你只是想自己补充用，可以在skill或prompt里补上一句：“常见各游戏数据库有~”。
...
**本站提供的资源均来自网络，版权争议与本站无关。© 2026 梅影寒窗。允许个人使用、修改、分发。严禁出售及商业用途。（除非我自己做了集成软件）**
