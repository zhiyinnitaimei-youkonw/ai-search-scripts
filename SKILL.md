...
description: Free aggregated web search and page scraping on a mobile QuickJS runtime via search_web() and scrape_web(). Use whenever the user needs to look something up online — facts, news, study or coding topics, game/anime/character info. Defines route tags, the game playbook, author filtering, game databases (Star Rail / Genshin), scraping rules, and return fields.
---
...
- `search_web(query)` — multi-engine aggregated search, routed by a leading `@` tag.
- `scrape_web(url)` — fetch + extract a page (or up to 5).
...
- ✅ `@game 爱莉`  ·  ✅ `@game 原神 6.6 前瞻`  ·  ✅ `@game 昔涟 培养攻略`
...
- **Enforced in-script:** the engine hard-caps to the **first 3** space-separated keywords and returns the rest in `droppedKeywords`. If you see `droppedKeywords` in a result, your query was over-long — the search ran on a truncated query, so don't trust it to be precise; re-issue with ≤3 deliberately-chosen words (the cap keeps the *first* 3, which may not be the ones you needed).
...
| | character | `starrailstation.com/cn/character/{pageId}` | stats, skills, eidolons, traces, relic rec, stories, voice (from `window.PAGE_CONFIG`) |
...
- Genshin has **no** roster-with-rarity page like Star Rail; meropide gives skill **multipliers**, not star/element/constellations. For Genshin star/element, use the game Wiki (`bwiki_ys`) or the official preview — and per Rule 2, mark it if you can't confirm.
...
- `moegirl` / `bwiki_*` / `prts_wiki` — Wiki API title + snippet.
...
**Worth scraping:** `bing` / `bing-en` hits (stub only), full `moegirl` / `bwiki_*` pages (now extracted correctly via MediaWiki content block), the game DB pages above. Pick the **1–2 most relevant** URLs.
...
If a `scrape_web` `error` starts with `SPA(...)`, abandon that URL.

## Return fields

`search_web` → `{ items: [{title, url, text, engine, spa?, summaryEmpty?, tag?, play?, pubdate?, author?}], query, routeTag, total, dictPolluted?, droppedKeywords? }`
...
- `droppedKeywords: [...]` = your query exceeded 3 words; the engine searched only the first 3 and **dropped these**. The result is on a truncated query — re-issue with ≤3 words you actually chose.
- `query` = the query the engine actually ran (already capped). Compare it to what you sent.

`scrape_web` → `{ urls: [{url, title, content, error}], stats: {total, ok, spa} }`
